...
with counts as (
    select
        ...
        (select count(*) from {{ ref('int_wolt_purchase_logs_curated') }}) as int_purchase_rows,
        (select count(distinct purchase_key) from {{ ref('int_wolt_purchase_logs_curated') }}) as int_purchase_distinct_purchase_key,
        ...
        (select count(*) from {{ ref('int_wolt_order_items_priced') }}) as int_priced_rows,
        (select countif(item_key_sk is null or item_scd_sk is null) from {{ ref('int_wolt_order_items_priced') }}) as int_priced_unmatched_item_rows,
        (select count(*) from {{ ref('int_wolt_order_items_promoted') }}) as int_promoted_rows,
