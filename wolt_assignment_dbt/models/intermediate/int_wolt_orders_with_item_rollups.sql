with orders as (
    ...
    from {{ ref('int_wolt_purchase_logs_curated_filtered') }}
),
order_item_metrics as (
    select
        ...
        sum(order_item_row_base_amount_gross_eur) as derived_order_items_base_amount_gross_eur,
        sum(order_item_row_discount_amount_gross_eur) as derived_order_items_discount_amount_gross_eur,
        sum(order_item_row_final_amount_gross_eur) as derived_order_items_final_amount_gross_eur
...
select
    ...
    coalesce(m.derived_order_items_base_amount_gross_eur, 0) as derived_order_items_base_amount_gross_eur,
    coalesce(m.derived_order_items_discount_amount_gross_eur, 0) as derived_order_items_discount_amount_gross_eur,
    coalesce(m.derived_order_items_final_amount_gross_eur, 0) as derived_order_items_final_amount_gross_eur,
